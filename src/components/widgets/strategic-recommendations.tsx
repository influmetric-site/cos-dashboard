...
export function StrategicRecommendations({ data }: StrategicRecommendationsProps) {
  const title = data?.title ?? "STRATEJİK BÜYÜME ANALİZİ & AI TAVSİYELERİ"
  const items = data?.items ?? defaultRecommendations
  ...
  return (
    <div className="bg-surface border border-white/5 p-8 rounded-[2.5rem] h-full flex flex-col transition-all duration-500 hover:border-white/10">
      <h3 className="text-[10px] font-black tracking-[0.3em] text-gray-500 uppercase mb-8 italic">{title}</h3>
          ...
          <tbody className="text-[11px]">
            {items.map((item) => (
              <tr key={item.id} className="group hover:bg-white/[0.03] transition-all duration-300">
                ...
                <td className={`py-5 pr-4 rounded-r-2xl text-right font-black italic uppercase tracking-tighter border-y border-right border-white/5 group-hover:border-white/10 ${item.statusColor || 'text-blue-400'}`}>
                  {item.status}
                </td>
              </tr>
            ))}
